...
#define EFUSE_POWER_INDEX_2G_LEN 11
#define EFUSE_POWER_INDEX_5G_LEN 14
#define EFUSE_POWER_INDEX_INVALID 0xFF

u8 _check_phy_efuse_tx_power_info_valid(PADAPTER padapter, int pwr_index_len, u16 index_offset)
{
	PHAL_DATA_TYPE pHalData = GET_HAL_DATA(padapter);
	...
	/*RTW_INFO("%s, pwr_index_len=%d, index_offset=%x\n", __func__, pwr_index_len, index_offset);*/

	if (index_offset) {
		for (index = 0 ; index < pwr_index_len ; index++) {
			if (pContent[index_offset + index] == 0xFF)
				ff_counter++;
			/*RTW_INFO("%s, index=%d, ff_counter=%d, pContent[index_offset + index]=%x\n", __func__, index, ff_counter,pContent[index_offset + index]);*/
		}
		if (ff_counter == 0)
			return _TRUE;
		else if (ff_counter == pwr_index_len)
			return _FALSE;
		else
...
int check_phy_efuse_tx_power_info_valid(PADAPTER padapter)
{
	...
	int index = 0;
	typedef	enum _INDEX_BAND_NUM {
		TX_INDEX_2G_A_PATH = 0,
		TX_INDEX_2G_B_PATH = 1,
		TX_INDEX_2G_C_PATH = 2,
		TX_INDEX_2G_D_PATH = 3,
		TX_INDEX_5G_A_PATH = 4,
		TX_INDEX_5G_B_PATH = 5,
		TX_INDEX_5G_C_PATH = 6,
		TX_INDEX_5G_D_PATH = 7,
		TX_INDEX_MAX_PATH = 8,
	} INDEX_BAND_NUM;
	u16 tx_index_offset[TX_INDEX_MAX_PATH];
	int pwr_index_len = 0;
	u8 ret = _FALSE;
	u8 result = _FALSE;

	memset(tx_index_offset, 0, TX_INDEX_MAX_PATH * sizeof(u16));

	switch (rtw_get_chip_type(padapter)) {
	case RTL8723B:
		tx_index_offset[TX_INDEX_2G_A_PATH] = EEPROM_TX_PWR_INX_8723B;
		break;
	case RTL8703B:
		tx_index_offset[TX_INDEX_2G_A_PATH] = EEPROM_TX_PWR_INX_8703B;
		break;
	case RTL8723D:
		tx_index_offset[TX_INDEX_2G_A_PATH] = EEPROM_TX_PWR_INX_8723D;
		break;
	case RTL8188E:
		tx_index_offset[TX_INDEX_2G_A_PATH] = EEPROM_TX_PWR_INX_88E;
		break;
	case RTL8188F:
		tx_index_offset[TX_INDEX_2G_A_PATH] = EEPROM_TX_PWR_INX_8188F;
		break;
	case RTL8192E:
		tx_index_offset[TX_INDEX_2G_A_PATH] = EEPROM_TX_PWR_INX_8192E;
		break;
	case RTL8821:
		tx_index_offset[TX_INDEX_2G_A_PATH] = EEPROM_TX_PWR_INX_8821;
		tx_index_offset[TX_INDEX_5G_A_PATH] = EEPROM_TX_PWR_INX_5G_A_8821;
		break;
	case RTL8812:
		tx_index_offset[TX_INDEX_2G_A_PATH] = EEPROM_TX_PWR_INX_8812;
		break;
	case RTL8814A:
		tx_index_offset[TX_INDEX_2G_A_PATH] = EEPROM_TX_PWR_INX_8814;
		break;
	case RTL8822B:
		tx_index_offset[TX_INDEX_2G_A_PATH] = EEPROM_TX_PWR_INX_8822B;
		break;
	case RTL8821C:
		tx_index_offset[TX_INDEX_2G_A_PATH] = EEPROM_TX_PWR_INX_8821C;
		tx_index_offset[TX_INDEX_2G_B_PATH] = EEPROM_TX_PWR_INX_2G_B_8821C;
		tx_index_offset[TX_INDEX_5G_A_PATH] = EEPROM_TX_PWR_INX_5G_A_8821C;
		break;
	default:
		tx_index_offset[TX_INDEX_2G_A_PATH] = 0x0010;
		break;
	}

	/* TODO: chacking length by ICs */

	for (index = 0 ; index < TX_INDEX_MAX_PATH ; index++) {
		if (index < TX_INDEX_5G_A_PATH)
			pwr_index_len = EFUSE_POWER_INDEX_2G_LEN;
		else
			pwr_index_len = EFUSE_POWER_INDEX_5G_LEN;

		ret = _check_phy_efuse_tx_power_info_valid(padapter, pwr_index_len, tx_index_offset[index]);
		/*RTW_INFO("%s, index=%d, ret=%x\n", __func__, index, ret);*/
		if (ret == EFUSE_POWER_INDEX_INVALID)
			return _FALSE;
		result = result | ret;
	}
	/*RTW_INFO("%s, result=%x\n", __func__, result);*/
